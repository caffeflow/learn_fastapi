...
@app04.post(
    ...
    description="This is description",  # 若为None,则显示请求函数的doc
    response_description="This is response description",
    deprecated=True,  # 接口已废弃
...
async def http_exception(city: str):
    if city != "Beijing":
        raise HTTPException(status_code=404, detail="City not found!", headers={
                            "X-Error": "Error"})  # 添加headers={"X-Error": "Error"}是在开发中常用方案
    return {"city": city}


@app04.get("/http_exception/{city_id}")
# 如果要重写HTTPException异常类，需要在主应用中重写
