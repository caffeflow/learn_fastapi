...
Fastapi的依赖注入系统：
    ...
    Fastapi主要使用函数接口,fastapi的依赖注入系统通过一些分层级的依赖函数(也可以依赖类)来提高代码的复用率，共享数据库的连接，增强安全、认证和角色管理。
    可以提高fastapi的兼容性：
...
fake_items_db = [{"item_name": "Foo"}, {
    "item_name": "Bar"}, {"item_name": "Baz"}]  # 这里只是模拟数据库中的数据
...
@app05.get("/classes_as_dependencies")  # 下面是3种类依赖的写法
